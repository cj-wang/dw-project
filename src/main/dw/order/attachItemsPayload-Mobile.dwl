...
fun transformSrd(dateString) = do {
  var indexOfX = (dateString splitBy "") indexOf "+"
  var getIndex = 
    if ((!isEmpty(dateString)) and (dateString[(indexOfX)+3] != ":"))
        dateString[0 to ((indexOfX)+2)] ++ ":" ++ dateString[((indexOfX)+3) to -1]
    else dateString
    ---
...
fun getServiceDOB(item) = do {
	var serviceDOBAttrib = (item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'serviceDateOfBirth')[0]."__text"
	---
	if(!isEmpty(serviceDOBAttrib))
		((serviceDOBAttrib as Date {format: "dd-MM-yyyy"}) as Date {format: "yyyy-MM-dd"}) as String ++ "T00:00:00"
	else
		null
}
fun readVariantAttrs(item) = do {
	if(!isEmpty(item))
...
fun filterLineItemsForContextKey(lineItems, contextKey) = do {
var lineItemsforContextKey = lineItems filter ($."@attribute-id" ~= "configuratorContextKey" and $."__text" ~= contextKey)
 ---
sizeOf(lineItemsforContextKey) > 0
}
fun filterLineItemsForOlive(lineItems) = do {

var oliveFlagFilter = lineItems filter ($."@attribute-id" ~= "oliveFlag" and $."__text" ~= "true")
---
sizeOf(oliveFlagFilter) > 0

}
fun lineItemForUpGradeProduct(contextKey) = do {
(rootElement."product-lineitems"."product-lineitem" filter filterLineItemsForContextKey($."custom-attributes"."custom-attribute", contextKey)) filter filterLineItemsForOlive ($."custom-attributes"."custom-attribute")
...
configContextKeyArr map (item1, index1) -> (
{
    ...
    "serviceRequestDate": transformSrd((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'serviceRequestDate')[0]."__text") as DateTime default "",
   ...
   "contractDuration": (item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'contractDuration')[0]."__text",
	...
	"calculationMatrixInput": [ readVariantAttrs((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'variantAttributes')[0]."__text") ],
	"productCode": (item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'masterProductCode')[0]."__text",
    ...
    "selectedSimType":  if ( lower((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'selectedSimType')[0]."__text") == "physical" ) "USIM"
                        ...
                        else if (networkTechnology == "HWBB") "USIM"
	...
	mobileSpecs: ((rootElement."product-lineitems"."product-lineitem") map (item, index) -> ({
		"configKey": (item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'configuratorContextKey')[0]."__text",
        "itemId": (convertObjectToArray(item."bundled-product-lineitems"."bundled-product-lineitem"."custom-attributes"."custom-attribute") filter $."@attribute-id" == 'billingOfferId')[0]."__text",
    	"UpgradeProtect": sizeOf(lineItemForUpGradeProduct(item1)) > 0,
    	"isBasePlan": (item."custom-attributes"."custom-attribute" filter ($."@attribute-id" ~= "isBasePlan"))[0]."__text",
    ...
    "allocatedNumber" : if (((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'serviceKeepMobileNumber')[0]."__text" == "NO") and 
    						(lower((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'numberAllocationStatus')[0]."__text") == "allocated")) //C2BS_19710 additional condition for port in scenario
    							(item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'serviceMobileNumber')[0]."__text" replace " " with "" default null
    						else null,
    //updated mapping based on C2BS-20265						
    "rlcMsisdn": if ( ( ((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'serviceKeepMobileNumber')[0]."__text" == "NO") and 
    			(lower((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'numberAllocationStatus')[0]."__text") == "allocated") ) or
    			((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'serviceKeepMobileNumber')[0]."__text" == "YES") ) 
    				(item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'serviceMobileNumber')[0]."__text" replace " " with "" default null
    			else null,
    ...
    "serviceDateOfBirth": getServiceDOB(item),
    "selectedSimType":  if ( lower((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'selectedSimType')[0]."__text") == "physical" ) "USIM"
    					else if ( lower((item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'selectedSimType')[0]."__text") == "esim" ) "ESIM"
                        else if (networkTechnology == "HWBB") "USIM"
    					else (item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'selectedSimType')[0]."__text",
    ...
    "ppvVerifyResponse": (read((((item."custom-attributes"."custom-attribute" filter ($."@attribute-id" ~= "PPVVerifyResponse"))[0]."__text") default "{}"), "application/json")).status default null,			
    "hasShippable": (item."custom-attributes"."custom-attribute" filter $."@attribute-id" == 'hasShippable')[0]."__text" as Boolean default false
	})) filter (item1 == $.configKey) orderBy (! $.hasShippable) )[0])  // DOP-3109 take hasShippable=true as preference for 4G
	//end shipment data
